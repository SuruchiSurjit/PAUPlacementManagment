...
import ApproveStdDetail from '../methods/ApproveStdDetail';

class InboxApprove extends React.Component{
    constructor(props)
{
    super(props);   
    this.state={
...
handleApproveStdDetail=(e)=>{
  e.preventDefault();  
  const{loginId,stdTrackId,progId,stdId,stdInfoId}=this.state
  ApproveStdDetail(loginId,stdTrackId,progId,stdId,stdInfoId).then(resp=>{
    if(resp){
      if(resp.data[0]){
        this.setState({
          message:resp.data[0],
      ...
      <tbody>
        ...
        <Modal.Footer>
          ...
          <Button variant="primary" 
             ...
             onClick={this.handleApproveStdDetail}
          >
            Approve
          </Button>
        </Modal.Footer>
